...
export async function createLease(formData: FormData): Promise<FormState> {
...
export async function deleteLease(id: number): Promise<FormState> {
  return runAction(async () => {
    await requireAdmin();
...
export async function saveDepositSettlement(
  ...
  return runAction(async () => {
    ...
    if (existing?.status === "confirmed") {
      throw new ValidationError("확정된 정산은 수정할 수 없습니다.");
    }
...
export async function confirmDepositSettlement(
  leaseId: number,
): Promise<FormState> {
  return runAction(async () => {
    ...
    const settlement = await db
      ...
      .executeTakeFirst();
    if (!settlement) throw new ValidationError("정산 내역이 없습니다.");
