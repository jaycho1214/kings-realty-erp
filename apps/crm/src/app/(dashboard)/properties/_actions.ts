...
export async function createProperty(formData: FormData): Promise<FormState> {
  return runAction(async () => {
    ...
    const management_phone =
      (formData.get("management_phone") as string) || null;
    ...
    if (Number.isNaN(monthly_rent_krw) || Number.isNaN(deposit_krw)) {
      throw new ValidationError("임대료와 보증금을 숫자로 입력해주세요.");
    }
    if (!Number.isInteger(landlord_id) || landlord_id <= 0) {
      throw new ValidationError("임대인을 선택해주세요.");
    }
...
export async function updateProperty(
  ...
  return runAction(async () => {
    ...
    if (!address?.trim()) {
      throw new ValidationError("주소를 검색하여 선택해주세요.");
    }
    if (Number.isNaN(monthly_rent_krw) || Number.isNaN(deposit_krw)) {
      throw new ValidationError("임대료와 보증금을 숫자로 입력해주세요.");
    }
