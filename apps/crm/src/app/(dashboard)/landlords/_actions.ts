...
export async function createLandlord(formData: FormData): Promise<FormState> {
...
export async function deleteLandlord(id: number): Promise<FormState> {
  return runAction(async () => {
    ...
    if (Number(properties?.c ?? 0) > 0) {
      throw new ValidationError("매물이 연결된 임대인은 삭제할 수 없습니다.");
    }
