"use client";

import { useState, useTransition } from "react";
import { Copy, Check, RefreshCw, Globe } from "lucide-react";
import { cn } from "@/lib/utils";
import { Button } from "@/components/ui/button";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogDescription,
} from "@/components/ui/dialog";
import { generateCalendarToken } from "../_actions";

interface CalendarSubscribeDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

function CopyButton({ text }: { text: string }) {
  const [copied, setCopied] = useState(false);

  function handleCopy() {
    navigator.clipboard.writeText(text);
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  }

  return (
    <button
      type="button"
      onClick={handleCopy}
      className="flex size-8 shrink-0 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
    >
      {copied ? (
        <Check className="size-3.5 text-primary" />
      ) : (
        <Copy className="size-3.5" />
      )}
    </button>
  );
}

export function CalendarSubscribeDialog({
  open,
  onOpenChange,
}: CalendarSubscribeDialogProps) {
  const [token, setToken] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  function handleGenerateToken() {
    startTransition(async () => {
      const result = await generateCalendarToken();
      if (result.token) {
        setToken(result.token);
      }
    });
  }

  const baseUrl = typeof window !== "undefined" ? window.location.origin : "";
  const feedUrl = token
    ? `${baseUrl}/api/calendar/feed.ics?token=${token}`
    : null;
  const webcalUrl = feedUrl
    ? feedUrl.replace(/^https?:\/\//, "webcal://")
    : null;
  const googleUrl = webcalUrl
    ? `https://calendar.google.com/calendar/r?cid=${encodeURIComponent(webcalUrl)}`
    : null;

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>캘린더 구독</DialogTitle>
          <DialogDescription>
            외부 캘린더 앱에서 Kings Realty 일정을 구독할 수 있습니다.
          </DialogDescription>
        </DialogHeader>

        {!token ? (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              구독 링크를 생성하면 Apple Calendar, Google Calendar 등에서 일정을
              자동으로 받아볼 수 있습니다.
            </p>
            <Button
              onClick={handleGenerateToken}
              disabled={isPending}
              className="w-full gap-2"
            >
              {isPending ? <RefreshCw className="size-4 animate-spin" /> : null}
              구독 링크 생성
            </Button>
          </div>
        ) : (
          <div className="space-y-5">
            {/* Apple Calendar / iOS */}
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <svg className="size-4" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
                </svg>
                <h4 className="text-sm font-medium">Apple Calendar / iOS</h4>
              </div>
              <p className="text-xs text-muted-foreground">
                아래 링크를 Safari에서 열거나 캘린더 앱에 URL을 추가하세요.
              </p>
              <div className="flex items-center gap-2 rounded-lg border bg-muted/50 px-3 py-2">
                <code className="min-w-0 flex-1 break-all text-xs leading-relaxed">
                  {webcalUrl}
                </code>
                <CopyButton text={webcalUrl!} />
              </div>
            </div>

            {/* Google Calendar */}
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <svg className="size-4" viewBox="0 0 24 24">
                  <path
                    d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
                    fill="#4285F4"
                  />
                  <path
                    d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                    fill="#34A853"
                  />
                  <path
                    d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                    fill="#FBBC05"
                  />
                  <path
                    d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                    fill="#EA4335"
                  />
                </svg>
                <h4 className="text-sm font-medium">Google Calendar</h4>
              </div>
              <p className="text-xs text-muted-foreground">
                아래 버튼을 클릭하거나 Google Calendar에서 URL로 구독하세요.
              </p>
              <div className="flex gap-2">
                <Button
                  variant="outline"
                  size="sm"
                  className="flex-1"
                  onClick={() => window.open(googleUrl!, "_blank")}
                >
                  Google Calendar에서 구독
                </Button>
                <CopyButton text={feedUrl!} />
              </div>
            </div>

            {/* Direct URL */}
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <Globe className="size-4 text-muted-foreground" />
                <h4 className="text-sm font-medium">직접 URL</h4>
              </div>
              <div className="flex items-center gap-2 rounded-lg border bg-muted/50 px-3 py-2">
                <code className="min-w-0 flex-1 break-all text-xs leading-relaxed">
                  {feedUrl}
                </code>
                <CopyButton text={feedUrl!} />
              </div>
            </div>

            {/* Regenerate token */}
            <div className="border-t pt-4">
              <Button
                variant="ghost"
                size="sm"
                className="w-full gap-2 text-muted-foreground"
                onClick={handleGenerateToken}
                disabled={isPending}
              >
                <RefreshCw
                  className={cn("size-3.5", isPending && "animate-spin")}
                />
                토큰 재생성
              </Button>
              <p className="mt-1 text-center text-[11px] text-muted-foreground">
                재생성 시 기존 구독 링크는 더 이상 작동하지 않습니다.
              </p>
            </div>
          </div>
        )}
      </DialogContent>
    </Dialog>
  );
}
